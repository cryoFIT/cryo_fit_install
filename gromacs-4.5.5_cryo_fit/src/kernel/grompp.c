...
#define NFILE asize(fnm)
  ...
  if(ir->emsteps >= 1){
   ...
   if(ir->comm_mode != ecmNO)
     warning_note(wi, "CAREFUL! It is normally best to use comm_mode=none when using CryoFit\n" );
	

     int oldstep;
    if ( ir->emsteps % ir->nstxout !=0){
     warning_note(wi, "emsteps must be an integer multiple of nstxout!!\n" );
     oldstep=ir->emsteps;
     ir->emsteps= ir->emsteps+ ir->nstxout - ir->emsteps %  ir->nstxout;
    fprintf(stderr,"Changing the value of emsteps from %d to %d\n",oldstep,ir->emsteps);
   }


   if(ir->emwritefrequency >= 1)
    if (ir->emwritefrequency % ir->emsteps !=0){
     warning_note(wi,
                     "To ensure that the proper simulated maps are written to file\nemwritefrequency must be an integer multiple of emsteps.\n");

		     
     oldstep=ir->emwritefrequency;
     ir->emwritefrequency= ir->emwritefrequency + ir->emsteps - ir->emwritefrequency % ir->emsteps;
    fprintf(stderr,"Changing the value of emwritefrequency from %d to %d\n",oldstep,ir->emwritefrequency);
   }
  }
