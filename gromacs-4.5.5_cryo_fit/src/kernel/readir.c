...
#define CHECK(b) _low_check(b,err_buf,wi)
  ...
  if (EI_STATE_VELOCITY(ir->eI) && ir->ePBC == epbcNONE && ir->comm_mode != ecmANGULAR && ir->emsteps ==0 ) {
            ...
            warninp_t wi)
{
  ...
  CCTYPE("MDfit potential parameters");
  ITYPE ("emsteps",  ir->emsteps,  0);
  ITYPE ("emwritefrequency",  ir->emwritefrequency,  -1);
  RTYPE ("emweight", ir->emweight, 0);
  RTYPE ("emthresh", ir->emthresh, 123456);
  RTYPE ("emsigma",  ir->emsigma,  0);
  RTYPE ("emcutoff", ir->emcutoff, 0);
  STYPE ("emffgrps", mdfit,      NULL);
              ...
              warninp_t wi)
{
  ...
  nmdfit = str_nelem(mdfit,MAXPTR, ptr1);
  do_numbering(natoms,groups,nmdfit,ptr1,grps,gnames,egcMDFIT,
               restnm,egrptpALL_GENREST,bVerbose,wi);
  ir->opts.ngmdfit=nmdfit;
