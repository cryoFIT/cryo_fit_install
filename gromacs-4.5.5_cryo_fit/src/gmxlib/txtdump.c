                 ...
                 gmx_bool bMDPformat)
{
  ...
  if (available(fp,ir,indent,title)) {
    ...
    PI("emsteps", ir->emsteps);
    PR("emweight", ir->emweight);
    PR("emthresh", ir->emthresh);
    PR("emsigma", ir->emsigma);
    PR("emcutoff", ir->emcutoff);
    PR("emwritefrequency", ir->emwritefrequency);
